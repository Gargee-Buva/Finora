...
import { useCreateTransactionMutation , useUpdateTransactionMutation} from "@/features/transaction/transactionAPI";
import { useGetSingleTransactionQuery } from "@/features/transaction/transactionAPI";
...
const TransactionForm = (props: { 
  ...
  const { isLoading, data } = useGetSingleTransactionQuery(
  ...
  useEffect(() => {
    if (isEdit && transactionId && editData) {
      ...
      form.reset({
        ...
        type:
        editData.type === _TRANSACTION_TYPE.INCOME
        ...
        frequency: editData.isRecurring ? editData.recurringInterval : null,
  ...
  const payload = {
    ...
    // ✅ FIX AMOUNT (currency-safe)
    amount: Number(String(values.amount).replace(/[^0-9.]/g, "")),

    date: values.date.toISOString(),

    isRecurring: values.isRecurring,

    // ✅ MATCH BACKEND ENUM (daily | weekly | monthly | yearly)
